...
0.5.83 - Fixes the extracted asset view's missing default settings dependency and adds a runtime module regression check.

0.5.82 - Adds direct ZIP/folder export submenus, keeps issue confirmation only when needed, improves placement beside Obsidian's PDF export, and extracts the asset sidebar view from the plugin entry.

0.5.81 - Removes unused translations and styles, centralizes Markdown attachment-source collection, parallelizes diagnostic item probes, and trims generated bundle whitespace.

0.5.80 - Adds the note-menu share-package action and removes retired full-vault batch tools and legacy repair commands.

0.5.79 - Adds current-note share-package export as a folder or ZIP, including resolved OE Link and local attachments with missing-file and rename reporting.
